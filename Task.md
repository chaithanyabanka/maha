...
Load the contact page in a standard desktop browser at 100% zoom.

Observe the robot image on the right side of the form.

The image is partially hidden and cannot be viewed fully unless zoomed out to 50%.

Try to scroll horizontally or vertically; no scroll bar appears to access the rest of the image.
...
Tab into or click each input field and textarea.

Note that on desktop, no visible border or color change occurs during focus.

Repeat the test on a mobile device; the border turns light orange.
...
Observe the position of the 'Done' button relative to the form fields.

Notice the button is not directly grouped or aligned with Full name, Email, and Message boxes.
...
4. Form Labels and Image Alt Attributes (summary for remediation)
Labels on Inputs:
Every form control must have a corresponding <label> tag or an aria-label:
...
If the robot is decorative: <img src="..." alt="" />

If meaningful: <img src="..." alt="Cartoon robot character" />
